...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!parseResult.success) {
      logApiError('stripe-checkout', new Error(parseResult.error || 'Invalid JSON'), requestId)
      return createErrorResponse(
    ...
    if (!parsed.success) {
      ...
      logApiError('stripe-checkout', new Error(message), requestId, { validationErrors: parsed.error.issues })
      return createErrorResponse(
        new Error(message),
        requestId,
        400,
    ...
    if (bookingError || !booking) {
      logApiError('stripe-checkout', new Error('Booking not found'), requestId, { bookingId })
      return createErrorResponse(
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `Booking with ${booking.kings?.profiles?.full_name || 'Companion'}`,
              description: `From ${new Date(booking.start_time).toLocaleString()} to ${new Date(
                booking.end_time
              ).toLocaleString()}`,
            },
