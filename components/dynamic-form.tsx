'use client'
// @ts-nocheck - Complex Zod v4 + react-hook-form generic type inference issues
...
export function DynamicForm<T extends z.ZodRawShape = z.ZodRawShape>({
  ...
  const defaultValues = Object.keys(schema.shape).reduce(
    (acc, key) => {
      ...
      if (
        getTypeName(getZodDef(originalFieldSchema as ZodTypeAny)) === 'ZodDefault' ||
        getTypeName(getZodDef(originalFieldSchema as ZodTypeAny)) === 'default'
      ) {
        acc[key] = getZodDef(originalFieldSchema as ZodTypeAny)['defaultValue']()
        return acc
      }

      const baseType = unwrapZodType(originalFieldSchema as ZodTypeAny)
  ...
  const form = useForm<z.infer<typeof schema>>({
    // @ts-expect-error - Complex generic type inference between Zod v4 and react-hook-form
    ...
    if (initialValues) {
      ...
      const processedInitialValues = schemaKeys.reduce(
        (acc, key) => {
          ...
          const baseFieldType = unwrapZodType(fieldDefFromSchema as ZodTypeAny)
