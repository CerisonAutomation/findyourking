...
describe('Rate Limiting', () => {
  ...
  it('allows requests within limit', () => {
    const result1 = checkRateLimit(identifier, { maxRequests: 3, windowMs: 60000 })
    expect(result1.allowed).toBe(true)
    expect(result1.remaining).toBe(2)
    
    const result2 = checkRateLimit(identifier, { maxRequests: 3, windowMs: 60000 })
  ...
  it('blocks requests over limit', () => {
    ...
    const result = checkRateLimit(identifier, { maxRequests: 2, windowMs: 60000 })
    expect(result.allowed).toBe(false)
    expect(result.remaining).toBe(0)
  })

  it('includes rate limit headers', () => {
    const result = checkRateLimit(identifier + '2', { maxRequests: 10, windowMs: 60000 })
