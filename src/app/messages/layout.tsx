...
async function fetchConversations(userId?: string): Promise<EnrichedConversation[]> {
    ...
    const { data, error } = await supabase.rpc('get_conversations_with_details', { p_user_id: userId });
...
function ConversationList() {
  ...
  const { data: conversations, isLoading } = useQuery({
      ...
      enabled: !!user,
      refetchInterval: 5000, // Refetch every 5 seconds to get latest messages
  });
  ...
  if (isLoading) {
    return <div className="p-4 flex justify-center items-center h-full"><Loader2 className="animate-spin" /></div>;
  }
            ...
            >
              <div className="relative">
                <Avatar className="h-12 w-12 border-2 border-transparent group-hover:border-primary">
                  <AvatarImage src={convo.otherUser.avatarUrl} alt={convo.otherUser.id} />
                  <AvatarFallback>{convo.otherUser.id.charAt(0)}</AvatarFallback>
                </Avatar>
                {isOnline && (
                ...
                <div className="flex justify-between items-center">
                  <h3 className="font-semibold truncate">{convo.otherUser.id}</h3>
                   <p className="text-xs text-muted-foreground whitespace-nowrap">
                    {convo.lastMessage?.created_at && formatDistanceToNowStrict(new Date(convo.lastMessage.created_at))} ago
                  </p>
                </div>
...
export default function MessagesLayout({
  ...
  return (
    <div className={cn("h-full grid", isChatPage ? "grid-cols-1 md:grid-cols-[300px_1fr]" : "md:grid-cols-[300px_1fr]")}>
      <aside className={cn("h-full md:flex flex-col border-r", isChatPage ? "hidden" : "flex")}>
        <div className="p-4">
      ...
      <main className={cn("bg-background", !isChatPage && "hidden md:block")}>{children}</main>
    </div>
  );
}
