...
import {
  Plus, Loader2, CalendarDays, MapPin, Users, Ticket,
  ChevronDown, X, Check, Clock, PartyPopper, Filter,
} from 'lucide-react';
...
const CATEGORIES: EventCategory[] = [
  'party','dinner','drinks','outdoor','sports','cultural','travel','gaming','music','casual','other',
];

const CATEGORY_EMOJI: Record<string, string> = {
  party: '🎉', dinner: '🍽️', drinks: '🍹', outdoor: '🌳', sports: '⚽',
  cultural: '🎨', travel: '✈️', gaming: '🎮', music: '🎵', casual: '☕', other: '✨',
};
    ...
    <div className="flex flex-col h-full">
      ...
      <Tabs value={tab} onValueChange={setTab} className="flex flex-col flex-1 overflow-hidden">
        ...
        <TabsContent value="discover" className="flex-1 overflow-y-auto mt-0">
          {isDiscoverLoading ? (
            <div className="flex justify-center py-12"><Loader2 className="animate-spin size-6 text-muted-foreground" /></div>
...
function RsvpRow({ event, onOpen, onCancel, isCancelling }: {
  ...
  const past = isPast(new Date(event.endAt as unknown as string ?? event.startAt as unknown as string));
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-muted/40 cursor-pointer" onClick={onOpen}>
...
function HostedRow({ event, onOpen, onCancel, isCancelling }: {
  ...
  const isCancelled = (event as unknown as { isCancelled?: boolean }).isCancelled;
  return (
    <div className="flex items-center gap-3 px-4 py-3 hover:bg-muted/40 cursor-pointer" onClick={onOpen}>
      <div className="size-10 rounded-xl bg-primary/10 flex items-center justify-center text-xl shrink-0">
        {CATEGORY_EMOJI[event.category] ?? '✨'}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[10px] text-muted-foreground">
            <Users className="size-3 inline mr-0.5" />{event.attendeeCount ?? 0} going
          </span>
...
function EventDetail({ event, userId, rsvpMutation, onClose }: {
  event:        EventWithMeta;
  userId:       string | undefined;
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-2.5 mt-3 text-sm">
        ...
        {event.description && (
          <p className="text-muted-foreground leading-relaxed mt-1">{event.description as unknown as string}</p>
        )}
      </div>
      ...
      {userId && (
        <div className="flex gap-2 mt-5">
          <Button
            ...
            onClick={() => rsvpMutation.mutate({ userId, status: 'going' })}
          >
            {event.myRsvp === 'going' ? <><Check className="mr-1.5 size-4" />Going!</> : 'Going'}
          </Button>
          <Button
            className="flex-1"
            variant={event.myRsvp === 'maybe' ? 'secondary' : 'outline'}
...
function CreateEventForm({ userId, onSuccess }: { userId: string; onSuccess: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium" htmlFor="evt-max">Max attendees</label>
          <Input
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium" htmlFor="evt-price">Price ($)</label>
          <Input
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        <div
          ...
          tabIndex={0}
          onKeyDown={(e) => e.key === ' ' && set('isPublic', !form.isPublic)}
        >
          <span className={cn(
        ...
        <span className="text-sm font-medium">{form.isPublic ? 'Public event' : 'Private event'}</span>
      </label>
      ...
      >
        {mutation.isPending
          ...
          : <><PartyPopper className="size-4 mr-2" />Create Event</>}
      </Button>
    </form>
  );
}
